...
\begin{algorithm}
    \label{evaluerTousLesArbres}
    \DontPrintSemicolon
    \KwData{$\mathcal N = \mathcal C \cup \mathcal O$, l'ensemble de noeuds admissibles.\\\hphantom{Données : }\emph{args}, les paramètres complémentaires ; rien par défaut.\\\hphantom{Données : }$S^*$, une variable globale que l'on peut utiliser dans tous les appels indépendants\\\hphantom{Données : }de cet algorithme et qui représente la meilleure solution trouvée pour l'instant.}
    \KwResult{$S^*$, le meilleur arbre de stratégie trouvé avec l'espérance de coût minimale.}
    \tcp{Parcours par tous les noeuds admissibles (s'il en existe au moins un)}
    \For {$n \in \mathcal N$} {
        \eIf {$args = \varnothing$} {
            \tcp{Le cas où l'on a appelé cette fonction la première fois}
            \emph{Évaluer}($\mathcal N \setminus \{n\}$, \emph{CreateArgs}(\emph{n}))
        }{
            \tcp{L'on connecte un noeud courant avec son parent qui est stocké avec la branche à remplir dans \emph{args}}
            \emph{Connecter}(\emph{n}, \emph{args})

            \tcp{L'on continue en remplissant des enfants d'un noeud \emph{n} si c'est bien possible}
            \emph{Évaluer}($\mathcal N \setminus \{n\}$, \emph{ActualiserArgs}(\emph{n}, \emph{args}))
        }
    }
    \If {$\vert \mathcal N \vert = 0$} {

        \eIf {Il nous reste des branches à remplir} {
            \tcp{Danc ce cas-là, l'on doit se retourner vers une dernière observation dont une branche l'on n'a pas encore rempli}
            \emph{Évaluer}(\emph{NoeudsAdmissiblesPrecedents}(args), \emph{ActualiserArgs}(\emph{n}, \emph{args}))
        }{
            \tcp{L'on a déjà rempli toutes les branches possibles alors l'on a fini la construction d'un arbre ; il faut alors calculer son espérance du coût}
            \tcp{L'on suppose que l'on stocke un arbre courant dans \emph{args} et que l'on peut l'obtenir en appelant une fonction dévouée}
            \emph{ec} $\longleftarrow EC(\varnothing, ObtenirArbreCourant(args))$
            
            $S^* \longleftarrow $ \leIf{$ec < EC(\varnothing, S^*)$}{\emph{ObtenirArbreCourant}(\emph{args})}{$S^*$}
        }
    }
    \caption{\emph{Évaluer}}
\end{algorithm}
...
\begin{algorithm}
    \label{bruteForceProgDyn}
    \DontPrintSemicolon
    \KwData{$\mathcal N = \mathcal C \cup \mathcal O$, l'ensemble de noeuds admissibles.\\\hphantom{Données : }p, la probabilité que le système ne marchait pas après avoir fait une action\\\hphantom{Données : }précédente ; 1 par défaut.\\\hphantom{Données : }E, un ensemble des évidences ; $\varnothing$ par défaut.}
    \KwResult{$S^*$, le meilleur arbre de stratégie trouvé avec l'espérance de coût minimale.}
    
    \tcp{S'il n'y a pas de noeuds admissibles alors on ne retourne rien}
    \If{$\vert \mathcal N \vert = 0$}{
        \Return{$\varnothing, 0$}
    }
    
    \tcp{Une initialisation par un arbre qui ne contient qu'un noeud de service}
    $S^* \longleftarrow \{ServiceNode \longrightarrow \varnothing\}$
    
    \tcp{Parcours par tous les noeuds admissibles (s'il en existe au moins un)}
    \For {$n \in \mathcal N$} {
        $ec  \longleftarrow p \cdot \text{coût}(n)$
        
        \tcp{Une initialisation de l'arbre courant $S_c$ qui a \emph{n} pour sa racine}
        $S_c \longleftarrow \{n \longrightarrow \varnothing\}$    
        
        \For {\emph{tous les résultats possibles} $r$ \emph{d'exécution d'action qui correspond à} $n$} {
            $pn \longleftarrow P(o_0 \neq \text{normal} \mid E \cup \{n = r, o_0 \neq \text{normal toujours}\})$
            
            $ec \longleftarrow ec + p \cdot \emph{connect}(S_c, \emph{n}, \emph{DynProg}(\mathcal N \setminus \{n\}, pn, E \cup \{n = r, o_0 \neq \text{normal toujours}\}))$
        }
    ...
    \Return{$S^*, EC(E, S^*)$}
    \caption{Résolution par la programmation dynamique}
\end{algorithm}
