...
Chaque coût de réparation est ainsi représenté par une paire donnant le coût minimal et le coût maximal, et on considère que le vrai coût de réparation est une variable aléatoire de loi uniforme sur cet intervalle. L'espérance d'un coût de réparation est ainsi la moyenne entre les coûts minimal et maximal correspondants. L'objectif est d'utiliser l'élicitation pour améliorer les connaissances sur les coûts de réparation, c'est-à-dire diminuer la taille des intervalles de valeur pour chaque coût. %, afin que l'algorithme de \emph{Troubleshooting} puisse ainsi proposer une séquence d'actions plus adaptée.
